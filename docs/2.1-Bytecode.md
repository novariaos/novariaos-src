...
- `PUSH(02)` low mid1 mid2 high : push 4 bytes as 32-bit value (little-endian)
...
- `JMP(30)` addr: unconditional jump (4-byte address)
- `JZ(31)` addr: pop value, jump if zero (4-byte address)
- `JNZ(32)` addr: pop value, jump if not zero (4-byte address)
- `CALL(33)` addr: function call (4-byte address)
- `RET(34)` : return
- **5 Instructions**
